...
class SeekingOperator:
    ...
    def orchestrate(self, user_objective: str, max_steps: int = 5) -> Dict[str, Any]:
        ...
        while step_count < max_steps:
            print(f"[DEBUG] Orchestration Step {step_count}/{max_steps}", flush=True)
            ...
            print(f"[DEBUG] Selected Agent: {plan.selected_agent}", flush=True)
            logger.info(f"Operator selected {plan.selected_agent}: {plan.reasoning}")
            
            if plan.selected_agent not in self.agent_registry:
                print(f"[DEBUG] FAILED: Agent {plan.selected_agent} not found in registry.", flush=True)
            ...
            if result.get("fsm_state") == "COMPLETED" or result.get("status") == "success":
                ...
                logger.info(f"Sub-Task {task_id} completed successfully. Continuing to next phase...")
                
                # Internalize the successful sequence if it's more than just one step
                if step_count > 0:
                    self.experiential_engine.internalize_successful_sequence(f"Sequence For: {user_objective[:20]}", user_objective)
                
                # Check if the result explicitly states the MAIN objective is complete
                # In this MVP, we consider the goal reached if an agent returns a 'FinalSeal' key
                if result.get("result", {}).get("FinalSeal") == "APPROVED":
                     logger.info("🏁 Overall Objective reached via FinalSeal!")
                     return {"status": "completed", "final_state": self.memory.get_execution_history()}
            else:
                 logger.warning(f"Sub-Task {task_id} did not indicate success. State: {result.get('status')}")
                 # For now, we continue and let the Operator decide the next move (e.g. retry or different agent)
