...
from microgravity import __version__, __logo__
from microgravity.config.schema import Config

app = typer.Typer(
    name="microgravity",
    help=f"{__logo__} microgravity - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".microgravity" / "history" / "cli_history"
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} microgravity[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} microgravity v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """microgravity - Personal AI Assistant."""
    pass
...
@app.command()
def onboard():
    """Initialize microgravity configuration and workspace."""
    from microgravity.config.loader import get_config_path, load_config, save_config
    from microgravity.config.schema import Config
    from microgravity.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} microgravity is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.microgravity/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]microgravity agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/microgravity#-chat-apps[/dim]")
...
I am microgravity, a lightweight AI assistant.
...
def _make_provider(config: Config):
    ...
    from microgravity.providers.custom_provider import CustomProvider
    ...
    from microgravity.providers.registry import find_by_name
    spec = find_by_name(provider_name)
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        console.print("[red]Error: No API key configured.[/red]")
        console.print("Set one in ~/.microgravity/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    from microgravity.config.loader import load_config, get_data_dir
    ...
    from microgravity.session.manager import SessionManager
    from microgravity.cron.service import CronService
    from microgravity.cron.types import CronJob
    from microgravity.heartbeat.service import HeartbeatService
    
    if verbose:
        import logging
        logging.basicConfig(level=logging.DEBUG)
    
    console.print(f"{__logo__} Starting microgravity gateway on port {port}...")
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from microgravity.bus.events import OutboundMessage
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show microgravity runtime logs during chat"),
):
    ...
    if logs:
        logger.enable("microgravity")
    else:
        logger.disable("microgravity")
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]microgravity is thinking...[/dim]", spinner="dots")
    ...
    if message:
        ...
        from microgravity.bus.events import InboundMessage
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".microgravity" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # microgravity/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall microgravity")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from microgravity.config.loader import get_data_dir
    from microgravity.cron.service import CronService
...
def cron_add(
    ...
    from microgravity.config.loader import get_data_dir
    from microgravity.cron.service import CronService
    from microgravity.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} microgravity Status\n")
    ...
    if config_path.exists():
        from microgravity.providers.registry import PROVIDERS
