...
logger = logging.getLogger("microgravity.vectorstore")
...
class _GeminiEmbedder:
    ...
    def embed(self, texts: list[str]) -> np.ndarray:
        ...
        response = embedding(
            model="gemini/embedding-001",  # Use a more stable base model for maximum compatibility
            input=texts,
...
def _pick_embedder():
    ...
    gemini_key = load_config().providers.gemini.api_key

    if gemini_key:
        # Priority models to try
        models_to_try = ["gemini/text-embedding-004", "gemini/embedding-001"]
        for model_name in models_to_try:
            try:
                embedder = _GeminiEmbedder(gemini_key)
                # Override the default model for this attempt
                def _embed_with_model(texts, m=model_name, k=gemini_key):
                    from litellm import embedding
                    response = embedding(model=m, input=texts, api_key=k)
                    embeddings = [item['embedding'] for item in response['data']]
                    matrix = np.array(embeddings, dtype=np.float32)
                    norms = np.linalg.norm(matrix, axis=1, keepdims=True)
                    norms[norms == 0] = 1e-10
                ...
                embedder.embed = _embed_with_model
                embedder.embed(["test"])
                logger.info("Using Gemini (%s) for vector embeddings", model_name)
                return embedder
            except Exception:
                continue
        
        logger.warning("Gemini embedding models not available or found (404). Falling back to local methods.")
