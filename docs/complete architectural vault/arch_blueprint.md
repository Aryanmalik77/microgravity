...
11. [UI Agent V2 & Kernel Architecture](#11-ui-agent-v2--kernel-architecture)
12. [Hybrid OS & Browser Automation](#12-hybrid-os--browser-automation)
...
## 11. UI Agent V2 & Kernel Architecture

The V2 upgrade introduces a resilient **KernelLoop** and **Introspection Engine** to replace the legacy linear execution flow.

### Key Kernel Modules:
- **IntrospectionEngine**: A triple-pass verification layer (Schema → Policy → Intent).
- **DiffusedAttentionMonitor**: Background sensory task for environmental anomaly detection.
- **Episodic Memory (LMDB)**: High-speed JSON storage for state-aware execution.

For detailed specifications, see: [`plan_ui_agent_v2_integration.md`](file:///c:/Users/HP/Downloads/micro%20gravity%20-%20Copy/docs/complete%20architectural%20vault/plan_ui_agent_v2_integration.md)

---

## 12. Hybrid OS & Browser Automation

MICROGRAVITY can now cross the OS/Browser boundary by combining DOM-based extraction (PinchTab) with Vision-based native control (UI Agent).

### Collaborative Flow:
1. **PinchTab**: Captures high-fidelity DOM state and extracts text.
2. **UI Agent**: Performs native OS-level actions (Move, Click, Type) to bypass detections and interact with non-DOM elements.
3. **Stagehand**: Provides high-level AI orchestration for complex browser tasks.

For detailed specifications, see: [`plan_hybrid_os_automation.md`](file:///c:/Users/HP/Downloads/micro%20gravity%20-%20Copy/docs/complete%20architectural%20vault/plan_hybrid_os_automation.md)
