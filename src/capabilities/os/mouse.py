...
from src.common.utils.math import generate_human_like_path
...
class ActionBlueprint:
    ...
    button: str = 'left'
    easing_func: str = 'linear'
    duration: Optional[float] = None
    hold_duration_ms: int = 0
    exit_velocity_px_s: Optional[float] = None
    axis_lock: Optional[str] = None

class MouseTool(Tool):
    """
    Standardized tool for mouse control with human-like imperfections.
    """
    @property
    def name(self) -> str:
    ...
    def description(self) -> str:
        return "Perform mouse actions (click, drag, scroll) with human-like movement."
    ...
    def parameters(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "action": {"type": "string", "enum": ["click", "double_click", "drag", "scroll", "move"]},
                "x": {"type": "integer"},
                "y": {"type": "integer"},
    ...
    async def execute(self, action: str, **kwargs: Any) -> str:
        """Executes the specialized mouse action."""
        x = kwargs.get("x")
        y = kwargs.get("y")
        ...
        if action == "click":
            if x is not None and y is not None:
                self.move_and_click(x, y, button=button)
            else:
            ...
            self.drag_to(x, y, button=button)
        elif action == "scroll":
            self.scroll(kwargs.get("amount", 200), direction=kwargs.get("direction", "down"))
        
        return f"Successfully executed mouse {action}"
